...
def create_html_page(product_info):
    ...
    safe_product_name = re.sub(r'[\\/*?:"<>|]', '', product_name).replace(' ', '_')[:50].strip('_')
    if not safe_product_name:
        safe_product_name = "product_" + str(hash(product_name + partner_url))[:8]
    html_filename = f"{safe_product_name}.html"
...
<style>
    ...
    .product-image img {{max-width:100%; height:auto; border-radius:8px; border:1px solid #eee; cursor:pointer;}}
    .product-detail p {{font-size:1.1em; line-height:1.6; margin-bottom:10px;}}
    .product-detail strong {{color:#2980b9;}}
    .buy-button {{display:block; width:80%; max-width:300px; margin:30px auto; padding:15px 25px; background:#ff5722; color:#fff; text-align:center; text-decoration:none; border-radius:8px; font-size:1.3em; font-weight:bold; transition:background-color 0.3s ease; box-shadow:0 4px 8px rgba(255,87,34,0.3);}}
    .buy-button:hover {{background:#e64a19; transform:translateY(-2px);}}
